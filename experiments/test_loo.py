...
model_fn = partial(model_fn, c=32, h=2, d=4)
...
if isinstance(F, PMatKFAC):
    ...
    for inputs, targets in tqdm(noisy_train_removed):
        ...
        for layer_id, layer in lc.layers.items():
            ...
            a_update_root = (
                J.permute(1, 3, 0, 2).reshape(J.shape[1], J.shape[3], -1)
    ...
    for inputs, targets in tqdm(noisy_train_removed):
        ...
        print(
            si.squeeze(),
            si_loo.squeeze(),
            torch.diag(si_loo_true.reshape(n_loo, n_loo)),
        )
        torch.testing.assert_close(
            si_loo.squeeze(), torch.diag(si_loo_true.reshape(n_loo, n_loo))
        )
